...
import StepPostTest1 from './StepPostTest1.vue';
import StepPostTest2 from './StepPostTest2.vue';
import StepThanks from './StepThanks.vue';
...
function saveProgress(s, patch) {
  ...
  fetch('/api/study2-subject/progress', {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({ step: s, subject_id: subjectId.value, name: name.value, group_type: 'process', data, submitted: s === 7 }),
  }).catch(() => {});
}
...
function onStepNext(nextStep) {
  if (nextStep === 3 && progressData.value.step2Cse) {
    fetch('/api/study2-subject/cse', {
      ...
      body: JSON.stringify({ subject_id: subjectId.value, group_type: 'process', scores: progressData.value.step2Cse }),
    }).catch(() => {});
  }
  step.value = nextStep;
...
function onStep1Next() {
  fetch('/api/study2-subject/register', {
    method: 'POST', headers: headers(),
    body: JSON.stringify({ subject_id: subjectId.value, name: name.value, group_type: 'process' }),
...
function onPostTest2Done(scores) {
  ...
  fetch('/api/study2-subject/posttest', {
    method: 'POST', headers: headers(),
    body: JSON.stringify({ subject_id: subjectId.value, group_type: 'process', scores: all }),
  }).catch(() => {});
  step.value = 7;
...
onMounted(() => {
  fetch(`/api/study2-subject/progress?group=process`, { headers: headers() })
    .then((r) => r.json())
    .then((data) => {
