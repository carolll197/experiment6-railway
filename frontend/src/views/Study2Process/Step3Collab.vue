        ...
        <div class="left-col panel-border">
          ...
          <div class="left-bottom panel-border ai-dialog">
            <div class="ai-header">
              ...
              <span class="ai-round-hint">（3轮对话结束后AI将生成方案供您参考）</span>
            </div>
            ...
            <div class="ai-input-bar" v-if="!chatDone">
...
import { briefQuestion1, kimiSystemPrompt, outputModules } from '../../content/study2Content.js';
...
async function sendUserMessage() {
  ...
  try {
    const apiMessages = [systemMessage, ...chatMessages.value.filter((m) => m.role !== 'system').map((m) => ({ role: m.role === 'user' ? 'user' : 'assistant', content: m.content }))];
    ...
    const isFinal = userRoundCount.value >= 3;
    chatMessages.value.push({ role: 'assistant', content: aiContent, isFinal });
    if (isFinal) {
      chatDone.value = true;
...
onMounted(() => {
  ...
  if (chatMessages.value.length === 0) {
    ...
    fetch('/api/study2-subject/chat', {
      ...
      .then((r) => r.json())
      .then((data) => {
        const content = data.choices?.[0]?.message?.content || '你好！我是你的广告创意助手。请告诉我你的创意想法。';
      ...
      .finally(() => { aiLoading.value = false; scrollChatBottom(); });
  }
  if (!timer) {
    timer = setInterval(() => {
      remaining.value--;
