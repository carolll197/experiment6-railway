...
function saveProgress(s, patch) {
  ...
  fetch('/api/study3-subject/progress', {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({ step: s, subject_id: subjectId.value, name: name.value, group_type: 'process', data, submitted: s === 5 }),
  }).catch(() => {});
}
...
onMounted(() => {
  fetch(`/api/study3-subject/progress?group=process`, { headers: headers() })
    .then((r) => r.json())
    .then((data) => {
